...
function computeBounds(features) {
  ...
  for (const f of features) eachPolygonCoords(f, ring => ring.forEach(([x,y]) => { minX=Math.min(minX,x); minY=Math.min(minY,y); maxX=Math.max(maxX,x); maxY=Math.max(maxY,y); }));
...
function abbrevFromProps(props={}) { return (props.postal||props.abbr||props.state||props.name||'').toString().toUpperCase(); }
...
function initializeGame() {
  ...
  for(let i=0;i<features.length;i++){ const ab=abbrevFromProps(features[i].properties); if(westCoastStates.has(ab)) startCandidates.push(i);}
...
function showQuestion(){
  ...
  currentQuestion={
    ...
    neighborId:gj.features[neighborState.index].id,
    currentId:gj.features[currentState.index].id,
    neighborIndex:neighborState.index,
    currentIndex:currentState.index
  };
  questionText.textContent=`Does ${neighborState.name} have a lower or higher ID number than ${currentState.name}?`;
...
function checkAnswer(isLower){
  ...
  const correctAnswer=currentQuestion.neighborId<currentQuestion.currentId;
